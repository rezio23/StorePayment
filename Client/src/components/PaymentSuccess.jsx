...
export default function PaymentSuccess() {
  ...
  if (status.loading) {
    return (
      <div className="max-w-xl mx-auto px-4 py-20 text-center">
        ...
        <h3 className="text-xl font-semibold text-slate-900">Verifying payment...</h3>
      </div>
    );
  }
  ...
  if (status.error) {
    return (
      <div className="max-w-xl mx-auto px-4 py-20 text-center">
        ...
        <h3 className="text-xl font-semibold text-slate-900 mb-2">Payment verification failed</h3>
        <p className="text-slate-500 mb-6">{status.error}</p>
        <Link
          to="/cart"
  ...
  return (
    <div className="max-w-xl mx-auto px-4 py-20 text-center">
      ...
      <p className="text-slate-500 mb-6">Thank you for your purchase. Your order has been confirmed.</p>

      <div className="bg-white rounded-2xl border border-slate-100 p-6 text-left mb-8">
        <div className="flex justify-between py-2 border-b border-slate-100">
          <span className="text-slate-500">Order ID</span>
          <span className="font-medium text-slate-900">{orderID || 'N/A'}</span>
        </div>
        <div className="flex justify-between py-2 border-b border-slate-100">
          <span className="text-slate-500">Session ID</span>
          <span className="font-medium text-slate-900 truncate max-w-[180px]">{sessionID || 'N/A'}</span>
        </div>
        <div className="flex justify-between py-2">
          <span className="text-slate-500">Total Paid</span>
          <span className="font-bold text-slate-900">${Number(GrandPrice || 0).toFixed(2)}</span>
        </div>
      </div>
